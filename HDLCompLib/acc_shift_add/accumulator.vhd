...
architecture FSM of accumulator is
    ...
    signal p_acc : signed(15 downto 0);
...
begin
  comb: process(in0, in0_empty, out0_full, count, p_acc, state)
    variable p_data : signed(15 downto 0);
  begin
    -- count=7, in0.rdy, state=ACC / p_data=in0, state:=RES, p_acc:=p_acc+p_data
    if count=to_unsigned(7,2) and in0_empty='0' and state=ACC then
      ...
      n_count <= count;
      en_count <= false;
    -- in0.rdy, state=ACC / p_data=in0, state:=ACC, p_acc:=acc+p_data, count:=count+1
      ...
      out0 <= (others => 'X');
      out0_wr <= '0';
    -- state=RES, out0.rdy / state:=ACC, wr(out0,acc), count:=0, p_acc:=0
    elsif state=RES and out0_full='0' then
      n_state <= ACC;
      en_state <= true;
      out0 <= std_logic_vector(p_acc);
      out0_wr <= '1';
